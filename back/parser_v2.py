"""
파일명: thetower/back/parser_v2.py
용도: data2 포맷 전투 리포트 텍스트 파싱 엔진
기능: V2 섹션 구조 파싱, BattleMainV2 / BattleDetailV2 데이터 생성
"""
from mappings import (
    KEY_MAP,
    SECTION_MAP,
    EXCLUDE_TOP_DAMAGE,
    V2_LINE_THRESHOLD,
    SECTION_MAP_V2,
    KEY_MAP_V2_REPORT,
    KEY_MAP_V2_RECORDS,
    DAMAGE_JSON_SECTIONS,
    STATS_JSON_SECTIONS,
)
from parser import parse_number, parse_date


def is_v2(text: str) -> bool:
    """
    줄 수를 기준으로 V2 포맷 여부 판별
    V2_LINE_THRESHOLD(100줄) 초과 시 V2로 판단
    """
    lines = [l for l in text.strip().split('\n') if l.strip()]
    return len(lines) > V2_LINE_THRESHOLD


def _get_val_robust(section_dict: dict, *keys: str, default: str = '0') -> str:
    """
    딕셔너리에서 대소문자 및 콜론(:) 구분 없이 값을 검색하여 반환합니다.
    """
    if not section_dict:
        return default
        
    # 모든 키를 소문자로 변환하고 끝의 콜론 제거하여 맵핑 생성
    normalized_dict = {k.lower().rstrip(':'): v for k, v in section_dict.items()}
    
    for key in keys:
        clean_target = key.lower().rstrip(':')
        if clean_target in normalized_dict:
            return normalized_dict[clean_target]
            
    return default


def parse_battle_report_v2(text: str) -> dict:
    """
    V2 포맷 리포트 텍스트를 파싱하여 반환
    """
    clean_text = text.replace('\r\n', '\n').replace('\r', '\n')
    lines = clean_text.split('\n')

    # 섹션별 raw 데이터 수집
    sections = {
        'report': {},
        'records': {},
        'damage': {},
        'damage_taken': {},
        'bonus_hp': {},
        'hp_regen': {},
        'damage_block': {},
        'utility': {},
        'stats': {},
        'enemy_hits': {},
        'kill_effects': {},
        'enemy': {},
        'coin': {},
        'cash': {},
        'currency': {},
        'kill_source': {},
    }

    current_section = 'report'

    for line in lines:
        line = line.strip()
        if not line:
            continue

        # V2 섹션 헤더 감지 (전체 문장 또는 콜론 제거 후 대조)
        clean_header = line.rstrip(':')
        if line in SECTION_MAP_V2:
            current_section = SECTION_MAP_V2[line]
            continue
        elif clean_header in SECTION_MAP_V2:
            current_section = SECTION_MAP_V2[clean_header]
            continue

        # Key-Value 파싱
        key, val = None, None
        if '\t' in line:
            parts = line.split('\t')
            key = parts[0].strip()
            val = parts[-1].strip()
        else:
            # 탭이 없는 경우에도 리포트 섹션의 주요 키는 인식할 수 있도록 처리
            if current_section == 'report':
                for check_key in ['Battle Date', '전투 날짜', 'Game Time', '게임 시간', 'Real Time', '실시간', 'Tier', '티어', 'Wave', '웨이브']:
                    if line.lower().startswith(check_key.lower()):
                        key = check_key
                        # 콜론이 있으면 건너뛰고 값 추출
                        val = line[len(check_key):].strip().lstrip(':').strip()
                        break
            
            if not key:
                parts = line.rsplit(' ', 1)
                if len(parts) == 2:
                    key, val = parts[0].strip(), parts[1].strip()

        if key and val is not None:
            sections[current_section][key] = val

    # ── BattleMain 데이터 구성 ──
    repo = sections['report']

    def get_repo(*keys, default='0'):
        return _get_val_robust(repo, *keys, default=default)

    battle_date = parse_date(get_repo('전투 날짜', 'Battle Date', default=''))

    main_data = {
        'battle_date': battle_date,
        'tier': get_repo('티어', 'Tier', default='T1'),
        'wave': int(get_repo('웨이브', 'Wave', default='0').replace(',', '')),
        'game_time': get_repo('게임 시간', 'Game Time', default=''),
        'real_time': get_repo('실시간', 'Real Time', default=''),
        'coin_earned': parse_number(get_repo('코인 획득', 'Coins Earned', 'Coins earned')),
        'coins_per_hour': parse_number(get_repo('시간당 코인', 'Coins Per Hour', 'Coins per hour')),
        'cells_earned': parse_number(
            _get_val_robust(sections['currency'], '획득한 셀', 'Cells Earned',
            default=get_repo('획득한 셀', 'Cells Earned', default='0'))
        ),
        'reroll_shards_earned': parse_number(
            _get_val_robust(sections['currency'], '다시 뽑기 파편 획득함', 'Reroll Shards Earned',
            default=get_repo('다시 뽑기 파편 획득함', 'Reroll Shards Earned', default='0'))
        ),
        'killer': get_repo('처치자', 'Killed By', default=''),

        # V2는 대미지 섹션이 분리됨
        'damage_dealt': _get_val_robust(sections['damage'], '입힌 대미지', 'Damage Dealt', 'Damage dealt'),
        'damage_taken': _get_val_robust(sections['damage_taken'], '타워', 'Tower', default='0'),

        # V2 적 합계
        'total_enemies': parse_number(_get_val_robust(sections['enemy'], '적 합계', 'Total Enemies', default='0')),

        # V2 효과 활성 상태에서 처치
        'death_wave_kills': parse_number(_get_val_robust(sections['kill_effects'], '죽음의 파동', 'Death Wave', default='0')),
        'spotlight_kills': parse_number(_get_val_robust(sections['kill_effects'], '스포트라이트', 'Spotlight', default='0')),
        'golden_bot_kills': parse_number(_get_val_robust(sections['kill_effects'], '황금 봇', 'Golden Bot', default='0')),

        # top_damages: V2 대미지 섹션 기준으로 계산
        'top_damages': _calculate_top_damages_v2(sections['damage']),
    }

    # ── BattleMainV2 데이터 구성 ──
    rec = sections['records']

    def get_rec(*keys, default='0'):
        return _get_val_robust(rec, *keys, default=default)

    main_v2_data = {
        'battle_date': battle_date,
        'cells_per_hour': parse_number(get_repo('시간당 셀', 'Cells Per Hour', 'Cells per hour')),
        'best_coins_per_minute': parse_number(get_rec('분당 최고 코인 수', 'Best Coins Per Minute', 'Highest Coins / Minute')),
        'max_wave_skip': parse_number(get_rec('최대 웨이브 건너뛰기', 'Max Wave Skip', 'Largest Wave Skip', default='0')),
        'best_skip_coins': parse_number(get_rec('웨이브 스킵에서 얻은 대부분의 코인', 'Most Coins From Wave Skip')),
        'best_skip_cells': parse_number(get_rec('웨이브 스킵에서 나온 대부분의 세포', 'Most Cells From Wave Skip', default='0')),
        'max_smart_missile_stack': parse_number(get_rec('최대 스마트 미사일 중첩', 'Max Smart Missile Stack', 'Largest Smart Missile Stack', default='0')),
        'max_golden_combo': parse_number(get_rec('최대 골든 콤보', 'Max Golden Combo', 'Largest Golden Combo', default='0')),
        'best_golden_combo_coins': parse_number(get_rec('골든 콤보에서 얻는 대부분의 코인', 'Most Coins From Golden Combo')),
        'max_inner_mine_charge': parse_number(get_rec('최대 내부 지뢰 충전', 'Max Inner Mine Charge', 'Largest Inner Landmine Charge', default='0')),
    }

    # ── BattleDetailV2 데이터 구성 ──
    damage_json = {k: v for k, v in sections.items() if k in DAMAGE_JSON_SECTIONS and v}
    stats_json = {k: v for k, v in sections.items() if k in STATS_JSON_SECTIONS and v}

    detail_v2_data = {
        'battle_date': battle_date,
        'damage_json': damage_json,
        'utility_json': sections['utility'],
        'stats_json': stats_json,
        'enemy_json': sections['enemy'],
        'coin_json': sections['coin'],
        'currency_json': sections['currency'],
        'kill_source_json': sections['kill_source'],
    }

    return {
        'main': main_data,
        'main_v2': main_v2_data,
        'detail_v2': detail_v2_data,
    }


def _calculate_top_damages_v2(damage_section: dict) -> list:
    """
    V2 대미지 섹션에서 top_damages 계산
    V2는 '투사체 대미지' 형태가 아닌 '투사체' 형태로 키가 존재함
    '입힌 대미지'와 숫자가 아닌 항목은 제외
    """
    import re
    candidates = []
    
    # 제외 목록 소문화
    exclude_list_lower = [ex.lower() for ex in EXCLUDE_TOP_DAMAGE]
    
    for key, val in damage_section.items():
        # 이름 정규화 (접미사 제거) 및 제외 목록 확인
        clean_name = re.sub(r'( 대미지| Damage| damage)$', '', key, flags=re.IGNORECASE)
        
        # 대소문자 무시 체크
        if clean_name.lower() in exclude_list_lower or key.lower() in exclude_list_lower:
            continue
        
        raw_val = parse_number(str(val))
        if raw_val > 0:
            candidates.append({'name': clean_name, 'raw': raw_val})

    candidates.sort(key=lambda x: x['raw'], reverse=True)
    return [item['name'] for item in candidates[:3]]
