...
## 🆕 최근 업데이트 상세 (2026.02 ~ 2026.04)

아래 항목은 `history.txt` 기준으로 2026-02-23부터 2026-04-19까지의 핵심 변경 사항을 기술적으로 정리한 내용입니다.

### 1) 전투 상세 페이지 정보 구조 개선
* 상세 페이지 레이아웃을 재정비하고, 스탯 그리드와 아코디언 영역을 분리/정리해 가독성과 탐색성을 개선했습니다.
* 코인/화폐 정보 표시 영역은 UI 묶음을 재구성해 의미 단위별 비교가 쉬워지도록 조정했습니다.

### 2) 데이터 정합성 및 삭제 처리 안정화
* 기록 삭제 관련 로직을 보강해 삭제 후 목록/상세 상태가 일관되게 반영되도록 개선했습니다.
* 사망 원인 집계에서 null 값을 제외하여 통계 노이즈를 줄이고 지표 신뢰도를 높였습니다.

### 3) 파서 및 저장 파이프라인 보강
* 데미지 파싱 결과의 저장 경로 및 처리 순서를 정리해 누락 가능성을 낮췄습니다.
* 저장 로직 오류를 수정해 특정 조건에서 저장이 반영되지 않던 이슈를 해소했습니다.
* 파서 오류 수정과 후속 안정화 작업을 통해 리포트 업로드 성공률을 높였습니다.

### 4) 레거시 제거 및 코드 구조 정리
* V1 저장 로직을 제거하여 중복 경로를 정리하고 유지보수 복잡도를 낮췄습니다.
* 프론트엔드는 파일 분리 리팩토링으로 기능 경계를 명확히 했고, 백엔드는 중복 코드를 정리했습니다.
* 타입 오류와 프론트 런타임 오류를 지속적으로 보정해 배포 안정성을 강화했습니다.

### 5) 표기 체계 및 사용자 경험 개선
* 영문 텍스트 개선 작업을 반복 적용해 다국어 품질을 보완했습니다.
* 초고단위 숫자 표기를 확장해 후반 성장 구간의 수치 표현 정확성을 강화했습니다.
* 에너지 쉴드 표시 개선 등 세부 시각 피드백을 보강했습니다.
